	...
	void MeshFromAttribute(Mesh& outMesh, cgltf_attribute& attribute, cgltf_skin* skin, cgltf_node* nodes, unsigned int nodeCount) {
		...
		for (unsigned int i = 0; i < acessorCount; ++i) {
			...
			case cgltf_attribute_type_joints:
			{
				...
				joints.x = joints.x < 0 ? 0 : GetNodeIndex(skin->joints[joints.x], nodes, nodeCount);
				joints.y = joints.y < 0 ? 0 : GetNodeIndex(skin->joints[joints.y], nodes, nodeCount);
				joints.z = joints.z < 0 ? 0 : GetNodeIndex(skin->joints[joints.z], nodes, nodeCount);
				joints.w = joints.w < 0 ? 0 : GetNodeIndex(skin->joints[joints.w], nodes, nodeCount);
